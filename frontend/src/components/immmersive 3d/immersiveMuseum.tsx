    ...
    useFrame((state) => {
        if (meshRef.current && hovered && lodLevel === 'high') {
            meshRef.current.position.y =
                Math.sin(state.clock.elapsedTime * 2) * 0.05 + 0.5;
    ...
    return (
        <group ref={groupRef} position={position}>
            ...
            <mesh position={[0, 0.25, 0]} castShadow receiveShadow>
                <cylinderGeometry
                    args={[0.6, 0.7, 0.5, lodLevel === 'low' ? 8 : 32]}
                />
                <meshStandardMaterial
...
function TeleportPoint({
    ...
    useFrame((state) => {
        if (meshRef.current) {
            meshRef.current.position.y =
                Math.sin(state.clock.elapsedTime * 3) * 0.1 + 0.2;
...
function AdvancedCameraController({
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            state.current.rotation.pitch -=
                e.movementY * params.lookSensitivity;
    ...
    useFrame((_, delta) => {
        ...
        if (tourMode && tourData) {
            ...
            rotation.pitch = THREE.MathUtils.lerp(
                rotation.pitch,
                targetPitch,
                3 * dt
            );
        ...
        if (movement.forward)
            targetVelocity.add(forward.clone().multiplyScalar(speed));
        if (movement.backward)
            targetVelocity.add(forward.clone().multiplyScalar(-speed));
        if (movement.left)
            targetVelocity.add(right.clone().multiplyScalar(-speed));
        if (movement.right)
            targetVelocity.add(right.clone().multiplyScalar(speed));
        ...
        if (isMoving) {
            ...
            const bobOffset =
                Math.sin(state.current.headBobPhase) * params.headBobAmplitude;
    ...
    return (
        <>
            ...
            <EffectComposer>
                ...
                <SSAO samples={16} radius={0.5} intensity={30} />
            </EffectComposer>
    ...
    const artifacts: Artifact[] = [
        {
            ...
            period: '1200 BC',
            description:
                'Traditional Kikuyu pottery with intricate geometric patterns representing community unity.',
            position: [-10, 0, -20],
        },
        {
            id: 2,
            ...
            period: '1800s',
            description:
                'Ceremonial warrior spear used in coming-of-age rituals.',
            position: [0, 0, -20],
        },
        {
            id: 3,
            ...
            period: '1900s',
            description:
                'Intricate Maasai beadwork with symbolic color meanings.',
            position: [10, 0, -20],
        },
        {
            id: 4,
            ...
            category: 'Art',
            period: '1600s',
            description: 'Ritual ceremonial mask used in harvest celebrations.',
            position: [-15, 0, -8],
        },
        {
            id: 5,
            ...
            period: '1400 BC',
            description:
                'Storage vessel for grains with rain-summoning symbols.',
            position: [-5, 0, -8],
        },
        {
            id: 6,
            ...
            category: 'Tools',
            period: '1700s',
            description: 'Blacksmith-crafted agricultural implements.',
            position: [5, 0, -8],
        },
        {
            id: 7,
            ...
            period: '1850s',
            description:
                'Hand-woven fabric with ancestral storytelling patterns.',
            position: [15, 0, -8],
        },
        {
            id: 8,
            ...
            period: '1750s',
            description: 'Traditional drum used in ritual ceremonies.',
            position: [-10, 0, 5],
        },
        {
            id: 9,
            ...
            category: 'Weapons',
            period: '1800s',
            description: 'Battle shield with clan identification markings.',
            position: [0, 0, 5],
        },
        {
            id: 10,
            ...
            period: '1900s',
            description:
                'Woven storage basket with traditional dye techniques.',
            position: [10, 0, 5],
        },
    ];
    ...
    const tourStops: TourStop[] = [
        {
            artifactId: 1,
            duration: 8,
            cameraPosition: [-8, 2, -18],
            cameraTarget: [-10, 0.5, -20],
            narration: 'Welcome to our Ancient Pottery exhibit...',
        },
        {
            artifactId: 2,
            duration: 8,
            cameraPosition: [2, 2, -18],
            cameraTarget: [0, 0.5, -20],
            narration: 'This Maasai spear represents...',
        },
        {
            artifactId: 3,
            duration: 8,
            cameraPosition: [12, 2, -18],
            cameraTarget: [10, 0.5, -20],
            narration: 'Beadwork is central to Maasai culture...',
        },
    ];
        ...
        <div className="w-full h-screen bg-black relative overflow-hidden">
                    ...
                    <div className="space-y-3">
                        <div className="p-4 bg-orange-500/10 border border-orange-500/30 rounded-lg">
                            <p className="text-orange-200 text-sm">
                                💡 <strong>Tip:</strong> Look for glowing blue
                                circles on the floor to teleport between gallery
                                sections
                            </p>
                        </div>
                        ...
                        <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
                            <p className="text-green-200 text-sm">
                                🎙️ <strong>Guided Tour:</strong> Click the radio
                                icon to start an automated tour with narration
                            </p>
                        </div>

                        <div className="p-4 bg-purple-500/10 border border-purple-500/30 rounded-lg">
                            <p className="text-purple-200 text-sm">
                                🥽 <strong>VR Mode:</strong> Experience the
                                museum in virtual reality (requires VR headset)
                            </p>
                        </div>
                    </div>
                </div>
            ...
            {tourMode && (
                <div className="absolute top-32 left-1/2 -translate-x-1/2 z-20 bg-black/90 backdrop-blur-md rounded-xl p-6 border border-green-500/30 max-w-lg">
                    ...
                    <div className="mb-4">
                        <div className="bg-white/5 rounded-full h-2 overflow-hidden">
                            <div
                                ...
                                style={{
                                    width: `${((currentTourStop + 1) / tourStops.length) * 100}%`,
                                }}
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            {currentTourStop < tourStops.length - 1
                                ? 'Next Stop'
                                : 'End Tour'}
                        </button>
                        <button
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                                        ...
                                        <p className="text-blue-200 text-sm">
                                            This artifact represents an
                                            important aspect of Kenyan cultural
                                            heritage, reflecting the
                                            craftsmanship and traditions of its
                                            time period.
                                        </p>
                                    </div>
            ...
            <div className="absolute bottom-6 left-6 z-20 space-y-2">
                <div className="px-4 py-2 bg-black/80 backdrop-blur-sm rounded-lg border border-white/20 text-white/80 text-sm">
                    Press{' '}
                    <kbd className="px-2 py-1 bg-white/20 rounded mx-1">
                        ESC
                    </kbd>{' '}
                    to release cursor
                </div>
