...
export const getAllStories = (req: Request, res: Response) => {
    ...
    if (culture) {
        ...
        if (!cultureStories) {
            return res.status(404).json({ message: `No stories found for culture: ${cultureKey}` });
        }
...
export const getStoryByTitle = (req: Request, res: Response) => {
    ...
    if (!cultureStories) {
        return res.status(404).json({ message: `Culture ${culture} not found` });
    }
    ...
    if (!story) {
        return res.status(404).json({ message: `Story titled "${title}" not found in ${culture}` });
    }
